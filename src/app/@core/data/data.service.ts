...
export class DataService {
  ...
  private setLoader(show) {
    if (show) {
      DataService.httpCallCount++;
      // NProgress.start();
    } else {
      DataService.httpCallCount--;
      if (DataService.httpCallCount == 0) {
        // NProgress.done();
      }
    }
  }

  private setNotification(message, type) {
    type = type === 'error' ? 'danger' : type;
    let $alert = $(`
            <div class="alert alert-${type} alert-notification" style="display:none;">
                ${message}
            </div>
        `);
    $('body').append($alert);
